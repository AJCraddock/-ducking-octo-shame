define(
    ...
    function(){
        ...
        function Map(player, objects, background){
            ...
            screens[this.player_index].push(this.player);
        ...
        Map.prototype = {
            ...
            update_screens: function(){
                var new_player_index = Math.floor(player.x/800);
                if(this.player_index != new_player_index){
                    ...
                    screens[new_player_index].push(this.player);
            ...
            get_nearby_objects: function(){
                ...
                if(this.player_index > 0){
                    for(var i = 0; i < this.screens[player_index-1].length; i++){
                        nearby_objects.push(this.screens[player_index-1][i]);
                    }
                }

                // add objects from the screen the player is on
                for(var i = 0; i < this.screens[player_index].length; i++){
                    if(this.screens[i] != this.player){
                        nearby_objects.push(this.screens[player_index][i]);
                    }
                }

                //add objects from the screen after the player's screen
                if(this.player_index < screens.length-1){
                    for(var i = 0; i < this.screens[player_index+1]; i++){
                        nearby_objects.push(this.screens[player_index+1][i]);
                    }
                }
